...
function switchPoint(name) {
  ...
  if (CURRENT_POINT) {
    ...
    const _prevName =
      CURRENT_POINT._mapKey ||
      String(
        CURRENT_POINT[PT.punto] || CURRENT_POINT['PUNTO'] || CURRENT_POINT['Punto'] || ''
      ).trim();
    if (_prevName) {
  ...
  document.getElementById('ctx-subtitulo').textContent =
    (p[PT.tipo] || '') + (p[PT.metodo] ? ' · ' + p[PT.metodo] : '');
  ...
  document.getElementById('hero-meta').innerHTML = meta
    .map(m => `<span class="hero-chip">${m}</span>`)
    .join('');
  ...
  updateTags('trabajo', cs);
  updateTags('domicilio', cs);
  // New heat-map style sections: reset to defaults (restoreCtxState will override if saved)
  (function () {
    var gi = document.getElementById('general-comp-input');
    if (gi) {
      gi.value = '';
    }
    var ri = document.getElementById('rutas-comp-input');
    if (ri) {
      ri.value = '';
    }
    updateTags('general', '');
    updateTags('rutas', '');
    var gc = document.getElementById('general-concentracion');
    if (gc) gc.innerHTML = 'Describe aquí las concentraciones del mapa de calor.';
    var rc = document.getElementById('rutas-concentracion');
    if (rc) rc.innerHTML = 'Describe aquí las rutas calientes de la zona.';
  })();

  // Restore images for this point (or clear if none)
  var imgSlots = {
    'map-main': 'img-mapa-main',
    'map-main2': 'img-mapa-main2',
    'map-trabajo': 'img-trabajo',
    'map-domicilio': 'img-domicilio',
    'map-general': 'img-general',
    'map-rutas': 'img-rutas',
    'map-overlap-1': 'img-overlap-1',
    'map-calificacion': 'img-cal'
  };
  ...
  allNavTabs.forEach(t => {
    if (t.getAttribute('onclick') && t.getAttribute('onclick').includes(sectionId)) targetTab = t;
  });
  ...
  if (
    !(
      CURRENT_POINT &&
      CURRENT_POINT._mapKey &&
      CTX_STORE[CURRENT_POINT._mapKey] &&
      CTX_STORE[CURRENT_POINT._mapKey].cmpTable
    )
  ) {
    buildComparables();
  }
  ...
  const _hasSavedCtx =
    CTX_STORE[name] && CTX_STORE[name].desc && CTX_STORE[name].desc.trim() !== '';
  if (!_hasSavedCtx) {
    if (lat && lng) fetchContext(name, lat, lng);
    else
      document.getElementById('ctx-descripcion').textContent =
        'Sin coordenadas. Edita este campo para agregar el contexto.';
  }
}

// AI CONTEXT
async function fetchContext(name, lat, lng) {
  document.getElementById('ctx-descripcion').innerHTML =
    '<span style="color:var(--muted);font-style:italic">⏳ Generando contexto del sitio con IA...</span>';
  try {
    ...
    document.getElementById('ctx-descripcion').textContent =
      text || 'No se pudo generar el contexto. Edita este campo.';
  } catch (e) {
    document.getElementById('ctx-descripcion').textContent =
      'Error al generar contexto. Edita este campo para agregar la descripción.';
  }
}
