function saveSession() {
  ...
  const html =
    '<!DOCTYPE html><!--REPORTE_SESSION:' +
    btoa(unescape(encodeURIComponent(json))) +
    '--><html><body><p style="font-family:sans-serif;padding:2rem;color:#666">Este archivo contiene una sesión guardada del Reporte de Puntos.<br>Ábrelo desde la pantalla de carga usando "Importar sesión guardada".</p></body></html>';
...
function importSession(input) {
  ...
  reader.onload = function (e) {
    try {
      ...
      if (sessionMatch) {
        ...
        if (session && session.version === 1) {
          ...
          _names.forEach(function (n) {
            if (!POINT_MAP[n]) {
              var d = { _mapKey: n };
              if (typeof PT !== 'undefined') d[PT.punto] = n;
              POINT_MAP[n] = [d];
              POINTS.push(d);
            }
          });
      ...
      const pointNames = Array.from(ptBtns)
        .map(function (b) {
          return b.textContent.trim();
        })
        .filter(Boolean);
      if (!pointNames.length) {
        alert('No se encontraron datos de puntos en este archivo HTML.');
        return;
      }
      ...
      pointNames.forEach(function (pName, i) {
        const pfx = 'pt' + i;
        function sec(s) {
          return doc.getElementById(pfx + '-' + s);
        }
        function txt(el) {
        ...
        const desc = ctxBody[1]
          ? ctxBody[1].querySelector('div[style]')
            ? ctxBody[1].querySelector('div[style]').innerHTML
            : ''
          : '';
        const fuenteEl = ctxBody[1]
          ? ctxBody[1].querySelector('[style*="font-size:.78rem"]')
          : null;
        ...
        const trbConc = trbSec
          ? trbSec.querySelector('.card-body div[style*="line-height"]')
            ? trbSec.querySelector('.card-body div[style*="line-height"]').innerHTML
            : ''
          : '';
        ...
        const domConc = domSec
          ? domSec.querySelector('.card-body div[style*="line-height"]')
            ? domSec.querySelector('.card-body div[style*="line-height"]').innerHTML
            : ''
          : '';
        ...
        const cmpImgSrc =
          cmpImgEl && cmpImgEl.src && cmpImgEl.src.startsWith('data:') ? cmpImgEl.src : null;
        const _cmpRows = cmpTableEl ? cmpTableEl.querySelectorAll('tbody tr') : [];
        const manualSelArr = Array.from(_cmpRows)
          .map(function (tr) {
        ...
        const ovlDescEl = (function () {
          if (!ovlSec) return null;
          var headers = ovlSec.querySelectorAll('.card-header');
          for (var h of headers) {
            if (h.textContent.trim() === 'Descripción') {
              var cb = h.nextElementSibling;
              return cb ? cb.querySelector('div[style]') || cb : null;
            }
          }
          return null;
        })();
        ...
        const ovlRows = [];
        if (ovlSec) {
          ovlSec.querySelectorAll('table tbody tr').forEach(function (tr) {
            const tds = tr.querySelectorAll('td');
            if (tds.length >= 3)
              ovlRows.push({
                tienda: tds[0].textContent.trim(),
                dist: tds[1].textContent.trim(),
                vis: tds[2].textContent.trim()
              });
          });
        }
        ...
        const calScoreEl = (function () {
          if (!calSec) return null;
          var inl = calSec.querySelector('#cal-score-inline');
          if (inl) return { textContent: inl.textContent };
          var spans = calSec.querySelectorAll('span[style*="border-radius"]');
          for (var s of spans) {
            if (/\d/.test(s.textContent)) return s;
          }
          return null;
        })();
        ...
        if (ctxImgEl && ctxImgEl.src.startsWith('data:'))
          IMG_STORE[pName + '__map-main'] = ctxImgEl.src;
        if (trbImgEl && trbImgEl.src.startsWith('data:'))
          IMG_STORE[pName + '__map-trabajo'] = trbImgEl.src;
        if (domImgEl && domImgEl.src.startsWith('data:'))
          IMG_STORE[pName + '__map-domicilio'] = domImgEl.src;
        if (calImgEl && calImgEl.src.startsWith('data:'))
          IMG_STORE[pName + '__map-calificacion'] = calImgEl.src;
        if (ovlImgEl && ovlImgEl.src.startsWith('data:'))
          IMG_STORE[pName + '__map-overlap-1'] = ovlImgEl.src;
        if (ovlImgEl && ovlImgEl.src && ovlImgEl.src.startsWith('data:') && pName) {
          if (!CTX_STORE[pName]) CTX_STORE[pName] = {};
          CTX_STORE[pName].imgOverlap = ovlImgEl.src;
          IMG_STORE[pName + '__map-overlap-1'] = ovlImgEl.src;
        }
        // Read cumplimiento images from export
        [15, 20, 30].forEach(function (mins) {
          var cw =
            doc.getElementById(pfx + '-cumpl-img-wrap-' + mins) ||
            doc.querySelector('#cumpl-img-wrap-' + mins);
          if (!cw) {
            // Also try inside the compliance section
            var cSec = sec(String(mins));
            cw = cSec
              ? cSec.querySelector('#cumpl-img-wrap-' + mins) ||
                cSec.querySelector('.map-placeholder')
              : null;
          }
          if (cw) {
        ...
        if (routes.length) {
          ROUTES[pName] = routes.map(function (r) {
        ...
        if (cmpImgSrc) {
          CTX_STORE[pName].compMapSrc = cmpImgSrc;
          IMG_STORE[pName + '__comp-map'] = cmpImgSrc;
        }
      ...
      if (firstPt) {
        ...
        var hasCumplimiento = tabs.some(function (t) {
          return t.textContent.trim() === 'Cumplimiento';
        });
...
function showImportConfirm(msg, savedAt) {
  ...
  el.innerHTML =
    '<div style="background:#e8f5e9;border:1.5px solid #a5d6a7;border-radius:8px;padding:.75rem 1rem;font-size:.83rem;color:#2e7d32;margin-top:.75rem">' +
    '✅ ' +
    msg +
    (savedAt
      ? '<br><span style="font-size:.75rem;opacity:.8">Guardado el ' +
