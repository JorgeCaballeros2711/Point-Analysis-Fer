...
function initZoomEvents() {
  ...
  wrap.addEventListener(
    ...
    e => {
      e.preventDefault();
      compMapScale = Math.max(0.5, Math.min(8, compMapScale * (e.deltaY > 0 ? 0.9 : 1.1)));
      applyTransform();
    },
    { passive: false }
  );
  wrap.addEventListener('mousedown', e => {
    isDragging = true;
    dragStartX = e.clientX - compMapX;
    dragStartY = e.clientY - compMapY;
    wrap.style.cursor = 'grabbing';
  });
  window.addEventListener('mousemove', e => {
    if (!isDragging) return;
    compMapX = e.clientX - dragStartX;
    compMapY = e.clientY - dragStartY;
    applyTransform();
  });
  window.addEventListener('mouseup', () => {
    isDragging = false;
    const w = document.getElementById('comp-map-wrap');
  ...
  wrap.addEventListener(
    ...
    e => {
      if (e.touches.length === 2)
        ...
        isDragging = true;
        dragStartX = e.touches[0].clientX - compMapX;
        dragStartY = e.touches[0].clientY - compMapY;
      }
    },
    { passive: true }
  );
  wrap.addEventListener(
    ...
    e => {
      if (e.touches.length === 2) {
        ...
        compMapScale = Math.max(0.5, Math.min(8, compMapScale * (d / ltd)));
        ltd = d;
        applyTransform();
      } else if (e.touches.length === 1 && isDragging) {
        compMapX = e.touches[0].clientX - dragStartX;
        compMapY = e.touches[0].clientY - dragStartY;
        applyTransform();
      }
    },
    { passive: true }
...
function applyTransform() {
  ...
  if (inner)
    inner.style.transform =
      'translate(' + compMapX + 'px,' + compMapY + 'px) scale(' + compMapScale + ')';
}
function zoomMap(f) {
  compMapScale = Math.max(0.5, Math.min(8, compMapScale * f));
  applyTransform();
}
function resetZoom() {
  compMapScale = 1;
