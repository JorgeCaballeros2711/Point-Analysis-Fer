...
dz.addEventListener('drop', e => {
  ...
  if (fi.files[0]) readXLSX(fi.files[0]);
});
...
function detectSheets() {
  ...
  CLASIFICACION.forEach(r => {
    const muni = String(r['Municipio'] || r['municipio'] || '')
      .trim()
      .toLowerCase();
    const cls = String(r['Clasificación'] || r['Clasificacion'] || r['clasificacion'] || '')
      .trim()
      .toLowerCase();
  ...
  GENERAL = { 15: [], 20: [], 30: [] };
  [15, 20, 30].forEach(m => {
    const exactG = ['Expansion Genie ' + m, 'EG ' + m],
      exactU = ['Expansion Genie ' + m + ' Urbano', 'EG ' + m + ' Urbano'],
      exactR = ['Expansion Genie ' + m + ' Rural', 'EG ' + m + ' Rural'];
    const gn =
      exactG.reduce((a, n) => a || ns.find(s => s.trim() === n), null) ||
      ns.find(n => {
        const nl = n.toLowerCase().replace(/[_\-]/g, ' ');
        return (
          nl.includes(String(m)) &&
          !nl.includes('rural') &&
          !nl.includes('urban') &&
          !nl.includes('urbano')
        );
    ...
    const un =
      exactU.reduce((a, n) => a || ns.find(s => s.trim() === n), null) ||
      findSheet(['urbano', 'urban', 'metropolit'], m);
    const rn =
      exactR.reduce((a, n) => a || ns.find(s => s.trim() === n), null) || findSheet(['rural'], m);
    RURAL[m] = rn ? sToJ(rn) || [] : [];
    URBAN[m] = un ? sToJ(un) || [] : [];
    GENERAL[m] = gn ? sToJ(gn) || [] : [];
  });
  ...
  const foundNames = {
    r15: findSheet(['rural'], 15),
    r20: findSheet(['rural'], 20),
    r30: findSheet(['rural'], 30),
  ...
  document.getElementById('sheet-rows').innerHTML =
    ...
    <span style="font-size:.75rem;color:var(--muted);margin-left:auto">${r.ok ? r.c + ' filas' : 'No encontrada'}</span></div>`
      )
      .join('') +
    '<div style="margin-top:.5rem;font-size:.73rem;color:var(--muted);border-top:1px solid var(--border);padding-top:.4rem">' +
    'Hojas detectadas: ' +
    Object.entries(foundNames)
      .map(([k, v]) => (v ? `<span style="color:var(--green)">${k}:${v}</span>` : null))
      .filter(Boolean)
      .join(', ') +
    '</div>';
  if (POINTS.length > 0) {
      ...
      if (_avBR.length <= 1) {
        [15, 20, 30].forEach(function (m) {
          ...
          if (sec) {
            ...
            if (t) t.textContent = 'Cumplimiento';
          }
        });
      }
    }
