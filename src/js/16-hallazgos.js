...
function generateHallazgos() {
  ...
  const localBench = isRuralPoint
    ? avgSheet(RURAL[firstMins] || [])
    : avgSheet(URBAN[firstMins] || []);

  const natPctPob = natBench && natBench.pob ? Math.round((pobPunto / natBench.pob) * 100) : 0;
  const localPctPob =
    localBench && localBench.pob ? Math.round((pobPunto / localBench.pob) * 100) : 0;
  ...
  const compNames = compRows
    .filter(function (r) {
      return !r.querySelector('[style*="PUNTO"]') && !r.querySelector('.comp-tag');
    })
    .map(function (r) {
  ...
  // Overlap data
  const overlapVis = document.getElementById('overlap-vis')
    ? document.getElementById('overlap-vis').value || 'X'
    : 'X';

  // Build text
  const text =
    'Al realizar la evaluación del punto para el proyecto de "' +
    corp +
    '", se identifican los siguientes hallazgos:\n\n' +
    'La población residente tiene un cumplimiento de ' +
    natPctPob +
    '% con respecto al promedio nacional y ' +
    localPctPob +
    '% respecto al promedio ' +
    benchType +
    ', por lo que la ubicación ' +
    cumpleDemo +
    ' con los requerimientos demográficos principales.\n\n' +
    'La tienda más cercana es "[escribe el nombre aquí]" por lo cual se calcula un impacto de Overlap Visitantes del ' +
    overlapVis +
    '%.\n\n' +
    'Se identifican como tiendas comparables por características demográficas: ' +
    (compNames.length ? compNames.join(', ') : '[sin comparables cargados]') +
    '.\n\n' +
