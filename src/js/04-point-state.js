function saveCtxState(pName) {
  ...
  document.querySelectorAll('#overlap-tbody tr').forEach(function (tr) {
    ...
    if (tds.length >= 3)
      ovlRows.push({ tienda: tds[0].value, dist: tds[1].value, vis: tds[2].value });
  });
  CTX_STORE[pName] = {
    desc:
      (function () {
        const el = document.getElementById('ctx-descripcion');
        const t = el ? el.textContent.trim() : '';
        if (!t || t.includes('Generando contexto') || t.includes('Cargando contexto')) return '';
    ...
    imgOverlap:
      CURRENT_POINT && CURRENT_POINT._mapKey
        ? IMG_STORE[CURRENT_POINT._mapKey + '__map-overlap-1'] || null
        : null,
    calEdits:
      CURRENT_POINT && CURRENT_POINT._mapKey
...
function restoreCtxState(pName) {
  ...
  if (!s) {
    ...
    const _p = document.getElementById('overlap-pct');
    if (_p) _p.value = '0';
    const _v = document.getElementById('overlap-vis');
    if (_v) _v.value = '0';
    const _pd = document.getElementById('overlap-pct-display');
    if (_pd) _pd.textContent = '0%';
    const _vd = document.getElementById('overlap-vis-display');
    if (_vd) _vd.textContent = '0%';
    const _tb = document.getElementById('overlap-tbody');
    if (_tb) {
      _tb.innerHTML = '';
      addOverlapRow('', '', '');
    }
  ...
  if (s.fuente) {
    const el = document.getElementById('ctx-fuente');
    if (el) el.value = s.fuente;
  }
  if (s.accDesc) {
    const el = document.getElementById('acc-descripcion');
    if (el) el.innerHTML = s.accDesc;
  }
  if (s.trbConc) {
    const el = document.getElementById('trabajo-concentracion');
    if (el) el.innerHTML = s.trbConc;
  }
  if (s.domConc) {
    const el = document.getElementById('domicilio-concentracion');
    if (el) el.innerHTML = s.domConc;
  }
  if (s.trbComp) {
    const el = document.getElementById('trabajo-comp-input');
    if (el) {
      el.value = s.trbComp;
      updateTags('trabajo', s.trbComp);
    }
  }
  if (s.domComp) {
    const el = document.getElementById('domicilio-comp-input');
    if (el) {
      el.value = s.domComp;
      updateTags('domicilio', s.domComp);
    }
  }
  if (s.genConc) {
    const el = document.getElementById('general-concentracion');
    if (el) el.innerHTML = s.genConc;
  }
  if (s.rutConc) {
    const el = document.getElementById('rutas-concentracion');
    if (el) el.innerHTML = s.rutConc;
  }
  if (s.genComp) {
    const el = document.getElementById('general-comp-input');
    if (el) {
      el.value = s.genComp;
      updateTags('general', s.genComp);
    }
  }
  if (s.rutComp) {
    const el = document.getElementById('rutas-comp-input');
    if (el) {
      el.value = s.rutComp;
      updateTags('rutas', s.rutComp);
    }
  }
  if (s.compDesc) {
    const el = document.getElementById('comparables-desc');
    if (el) el.textContent = s.compDesc;
  }
  if (s.ovlDesc) {
    const el = document.getElementById('overlap-desc');
    if (el) el.innerHTML = s.ovlDesc;
  }
  const _pctEl = document.getElementById('overlap-pct');
  if (_pctEl) _pctEl.value = s.ovlPct || '0';
  const _visEl = document.getElementById('overlap-vis');
  if (_visEl) _visEl.value = s.ovlVis || '0';
  const _pctDisp = document.getElementById('overlap-pct-display');
  if (_pctDisp) _pctDisp.textContent = (s.ovlPct || '0') + '%';
  const _visDisp = document.getElementById('overlap-vis-display');
  if (_visDisp) _visDisp.textContent = (s.ovlVis || '0') + '%';
  ...
  const togOvl = document.getElementById('tog-overlap');
  if (togOvl) togOvl.checked = _ovlVis;
  const togCal = document.getElementById('tog-cal');
  if (togCal) togCal.checked = _calVis;
  if (s.calScore) {
    const el = document.getElementById('cal-score');
    if (el) {
      el.value = s.calScore;
      updateScoreDisplay();
    }
  ...
  if (s.cumplEdits) {
    CUMPL_EDITS = JSON.parse(JSON.stringify(s.cumplEdits));
  }
  if (s.puntoEdits && pName) {
    if (!PUNTO_EDITS[pName]) PUNTO_EDITS[pName] = {};
    Object.assign(PUNTO_EDITS[pName], s.puntoEdits);
  }
  if (s.calEdits && pName) {
    CAL_EDITS[pName] = JSON.parse(JSON.stringify(s.calEdits));
  }
  if (typeof updateCalTable === 'function') setTimeout(updateCalTable, 0);
  if (s.cumplImgs && pName) {
    Object.keys(s.cumplImgs).forEach(function (m) {
      IMG_STORE[pName + '__cumpl-' + m] = s.cumplImgs[m];
    });
  }
  if (s.puntoEdits && pName) {
    if (!PUNTO_EDITS[pName]) PUNTO_EDITS[pName] = {};
  ...
  if (s.cmpTable) {
    ...
    if (s.imgOverlap && pName) {
      IMG_STORE[pName + '__map-overlap-1'] = s.imgOverlap;
      var _ovImg = document.getElementById('map-overlap-1');
      if (_ovImg && typeof renderImg === 'function')
        renderImg(
          'map-overlap-1',
          pName + '__map-overlap-1',
          document.getElementById('img-mapa-overlap-1')
        );
    }
    renderCompMap(pName);
  } else {
  ...
  if (s.calDesc) {
    ...
    if (el) el.innerHTML = s.calDesc;
  }
  if (s.hall) {
    const el = document.getElementById('hallazgos-texto');
    if (el) el.innerHTML = s.hall;
  }
